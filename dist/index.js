...
app.post('/api/auth/login', async (c) => {
    ...
    try {
        ...
        const fallback = fallbackUsers.find((candidate) => candidate.email.toLowerCase() === email && candidate.role === role);
        if (fallback) {
            user = fallback;
        }
    }
...
app.get('/api/queues/feed', async (c) => {
    try {
        ...
        for (const service of services) {
            ...
            const lastRows = await query(`SELECT queue_number FROM queue WHERE service_id = $1 ORDER BY queue_number DESC LIMIT 1`, [service.id]);
            feed.push({
...
app.post('/api/queues/create', async (c) => {
    ...
    try {
        const lastRow = await query(`SELECT queue_number FROM queue WHERE service_id = $1 ORDER BY queue_number DESC LIMIT 1`, [serviceId]);
        const nextNumber = (lastRow[0]?.queue_number ?? 0) + 1;
        const insertResult = await query(`INSERT INTO queue (student_id, service_id, counter_id, queue_number, status, created_at) VALUES ($1, $2, $3, $4, 'waiting', NOW()) RETURNING *`, [studentId, serviceId, 1, nextNumber]);
...
app.get('/api/queues/by-service/:serviceId', async (c) => {
    ...
    try {
        const rows = await query(`SELECT * FROM queue WHERE service_id = $1 ORDER BY queue_number ASC`, [serviceId]);
        return c.json({ success: true, data: rows });
    }
    catch (error) {
...
app.get('/api/queues/counter/:counterId', async (c) => {
    const counterId = Number(c.req.param('counterId'));
    try {
        const rows = await query(`SELECT * FROM queue WHERE counter_id = $1 ORDER BY queue_number ASC`, [counterId]);
        return c.json({ success: true, data: rows });
    }
    catch (error) {
...
app.post('/api/queues/next', async (c) => {
    ...
    try {
        const nextRow = await query(`SELECT * FROM queue WHERE service_id = $1 AND status = 'waiting' ORDER BY queue_number ASC LIMIT 1`, [serviceId]);
        ...
        const targetQueue = fallbackQueueStore.find((item) => Number(item.service_id) === serviceId && item.status === 'waiting');
        if (!targetQueue) {
            return c.json({ success: true, data: { serving: null, summary: { current_number: 0 } } });
        }
...
app.get('/api/queue/history', async (c) => {
    const rows = await query(`SELECT * FROM queue ORDER BY created_at DESC LIMIT 20`);

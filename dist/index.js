...
const fallbackQueueStore = [];
const fallbackQueueLogs = [];
...
initDbTables();
async function recordQueueLog(serviceId, queueNumber, action, previousNumber = null) {
    ...
    try {
        await query(`INSERT INTO queue_logs (service_id, queue_number, action, previous_number, created_at) VALUES ($1, $2, $3, $4, NOW())`, [normServiceId, normQueueNumber, normAction, normPrev]);
    }
    catch (error) {
...
app.post('/api/auth/login', async (c) => {
    ...
    let user = null;
    try {
        const rows = await query(`SELECT * FROM users WHERE email = $1 AND role = $2 LIMIT 1`, [email, role]);
        user = rows[0] ?? null;
        ...
        if (!user) {
            ...
            const inserted = await query(`INSERT INTO users (name, email, password_hash, role) VALUES ($1, $2, $3, $4) RETURNING *`, [formattedName, email, password, role]);
            user = inserted[0];
            if (user && role === 'student') {
                const studentNum = `STU-${Math.floor(1000 + Math.random() * 9000)}`;
                await query(`INSERT INTO students (user_id, student_number, department, year_level) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING`, [user.id, studentNum, 'BSIT', 'College']);
            }
        }
    }
    catch (error) {
        // Fallback to in-memory demo users if database is down or disconnected
        let fallback = fallbackUsers.find((candidate) => candidate.email.toLowerCase() === email && candidate.role === role);
        if (!fallback) {
            const rawName = email.split('@')[0];
            const formattedName = rawName
                .split(/[._-]/)
    ...
    if (user.role === 'student') {
        try {
            const studentRows = await query(`SELECT id FROM students WHERE user_id = $1 LIMIT 1`, [user.id]);
...
app.post('/api/auth/forgot-password', async (c) => {
    ...
    await query(`INSERT INTO password_resets (email, token, expires_at) VALUES ($1, $2, $3) ON CONFLICT (token) DO NOTHING`, [email, token, expiresAt]);
...
app.get('/api/auth/verify-reset-token', async (c) => {
    ...
    const rows = await query(`SELECT email FROM password_resets WHERE token = $1 AND expires_at > NOW() LIMIT 1`, [token]);
    const reset = rows[0];
    if (!reset) {
...
app.get('/api/services/counter/:counterId', async (c) => {
    ...
    const rows = await query(`SELECT * FROM services WHERE counter_id = $1 ORDER BY id ASC`, [counterId]);
    return c.json({ success: true, data: rows });
});
app.get('/api/students/user/:userId', async (c) => {
    const userId = Number(c.req.param('userId'));
    try {
        const rows = await query(`SELECT * FROM students WHERE user_id = $1 LIMIT 1`, [userId]);
...
app.get('/api/queues/feed', async (c) => {
    try {
        ...
        for (const service of services) {
            const currentRows = await query(`SELECT queue_number FROM queue WHERE service_id = $1 AND status = 'serving' ORDER BY served_at DESC NULLS LAST, created_at ASC LIMIT 1`, [service.id]);
            const waitingCount = await query(`SELECT COUNT(*)::int AS count FROM queue WHERE service_id = $1 AND status = 'waiting'`, [service.id]);
            const lastRows = await query(`SELECT queue_number FROM queue WHERE service_id = $1 ORDER BY created_at DESC, id DESC LIMIT 1`, [service.id]);
            let historyRows = [];
            try {
                historyRows = await query(`SELECT id, queue_number as number, action, previous_number, undone, created_at FROM queue_logs WHERE service_id = $1 ORDER BY created_at DESC, id DESC LIMIT 20`, [service.id]);
            }
            catch (err) {
                historyRows = [];
            }
...
app.post('/api/queues/create', async (c) => {
    ...
    try {
        const maxRow = await query(`SELECT COALESCE(MAX(queue_number), 0) AS max_num FROM queue WHERE service_id = $1`, [serviceId]);
        let nextNumber = (Number(maxRow[0]?.max_num) || 0) + 1;
        if (nextNumber > 999) {
            nextNumber = 1;
        }
        const insertResult = await query(`INSERT INTO queue (student_id, service_id, counter_id, queue_number, status, priority_type, created_at) VALUES ($1, $2, $3, $4, 'waiting', $5, NOW()) RETURNING *`, [studentId, serviceId, fallbackService?.counter_id ?? 1, nextNumber, priorityType]);
...
app.get('/api/queues/student/:studentId', async (c) => {
    ...
    try {
        const rows = await query(`SELECT * FROM queue WHERE student_id = $1 ORDER BY created_at DESC`, [studentId]);
...
app.get('/api/queues/by-service/:serviceId', async (c) => {
    ...
    try {
        const rows = await query(`SELECT * FROM queue WHERE service_id = $1 ORDER BY CASE WHEN status = 'waiting' AND priority_type != 'regular' THEN 0 WHEN status = 'waiting' THEN 1 WHEN status = 'serving' THEN 2 ELSE 3 END, created_at ASC, id ASC`, [serviceId]);
        return c.json({ success: true, data: rows.map(normalizeQueue) });
    }
    catch (error) {
        const rows = fallbackQueueStore
            ...
            const getRank = (item) => {
                if (item.status === 'waiting')
                    return (item.priority_type && item.priority_type !== 'regular') ? 0 : 1;
...
app.get('/api/queues/counter/:counterId', async (c) => {
    const counterId = Number(c.req.param('counterId'));
    try {
        const rows = await query(`SELECT * FROM queue WHERE counter_id = $1 ORDER BY created_at ASC, id ASC`, [counterId]);
        return c.json({ success: true, data: rows.map(normalizeQueue) });
    }
    catch (error) {
        const rows = fallbackQueueStore
...
app.post('/api/queues/next', async (c) => {
    ...
    try {
        const nextRow = await query(`SELECT * FROM queue WHERE service_id = $1 AND status = 'waiting' ORDER BY CASE WHEN priority_type != 'regular' THEN 0 ELSE 1 END, created_at ASC, id ASC LIMIT 1`, [serviceId]);
        const target = nextRow[0];
        if (!target) {
            return c.json({ success: true, data: { serving: null, summary: { current_number: 0 } } });
        }
        const prevServingRows = await query(`SELECT * FROM queue WHERE service_id = $1 AND status = 'serving' ORDER BY served_at DESC NULLS LAST LIMIT 1`, [serviceId]);
        ...
        const prevServing = fallbackQueueStore.find((item) => Number(item.service_id) === serviceId && item.status === 'serving');
...
app.post('/api/queues/set-priority', async (c) => {
    ...
    try {
        const updateResult = await query(`UPDATE queue SET priority_type = $1 WHERE id = $2 RETURNING *`, [priorityType, queueId]);
        if (!updateResult[0]) {
...
app.post('/api/queues/update-status', async (c) => {
    ...
    try {
        ...
        if (queueId) {
            const rows = await query(`SELECT * FROM queue WHERE id = $1 LIMIT 1`, [queueId]);
            targetQueue = rows[0];
        }
        else if (serviceId) {
            const rows = await query(`SELECT * FROM queue WHERE service_id = $1 AND status = 'serving' ORDER BY served_at DESC NULLS LAST LIMIT 1`, [serviceId]);
            targetQueue = rows[0];
        }
        if (targetQueue) {
            await query(`UPDATE queue SET status = $1, served_at = CASE WHEN $1 IN ('done', 'completed', 'served') THEN NOW() ELSE served_at END WHERE id = $2`, [status, targetQueue.id]);
...
app.post('/api/queues/undo', async (c) => {
    ...
    try {
        const lastNextRows = await query(`SELECT * FROM queue_logs WHERE service_id = $1 AND action = 'NEXT' AND undone = 0 ORDER BY created_at DESC, id DESC LIMIT 1`, [serviceId]);
...
app.get('/api/queue/history', async (c) => {
    try {
        const logs = await query(`SELECT * FROM queue_logs ORDER BY created_at DESC LIMIT 20`);
        if (logs.length > 0) {
            return c.json(logs.map((r) => ({
                id: r.id,
    ...
    if (fallbackQueueLogs.length > 0) {
        return c.json(fallbackQueueLogs.slice(0, 20).map((r) => ({
            id: r.id,
            queueNumber: r.queue_number,
            action: r.action,
            serviceId: r.service_id,
